...
- **Consent gate** (auto-discovery puts "borrowing" someone's mic one click away, so a human has to approve before the mic opens):
  ...
  - **Auto-follow pre-authorizes**: enabling auto-follow immediately sends a mic-free `GET /authorize` through the same consent gate, so the owner's dialog appears when you *set up* following — not mid-sentence, the moment an app first opens the mic. "Always allow" issues the usual token; "Allow once" leaves a one-shot pass redeemed by the next `/stream`; denial ends the follow session. Since a pre-auth request holds no microphone, it has no 30 s deadline — the waiting client heartbeats every ~2 s, and disconnecting (or 15 s of silence) auto-revokes the dialog. Servers without `/authorize` (pre-0.6) fall back to consent-at-use
