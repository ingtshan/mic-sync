...
MicSync 只解决这一件小事:**让接在一台 Mac 上的麦克风,变成局域网里其他 Mac 的"本机麦克风"**。并且要"平时安静":没人说话时不产生任何音频流量,说话瞬间自动接上,说完自动断开。

## 使用场景

- **双机位办公**:工作 MacBook + 个人台式 Mac 同一张桌子,麦克风接在其中一台;另一台要开会时,直接把会议软件的输入选成 BlackHole 即可,零插拔
- **多台 Mac 轮流开会**:同一个人先后在不同 Mac 上进会(公司环境 / 客户环境切换),所有客户端常开监听,坐到哪台前面就用哪台——说话即触发,无需任何手动切换
- **共享高品质录音链路**:电容麦 + 声卡接在录音/直播机位上,旁边的 Mac 需要临时进会时,直接借用这条链路的声音
- **多设备待机、不重复进声**:多台客户端可以同时监听同一个服务端,但同一时间只有一台真正收流(先到先得,串行轮转),不会把同一份人声同时送进两个会议
...
```
             ...
             │  MicSync「服务端」 :47800       │
             └───────────────┬───────────────┘
   GET /health 轮询(250ms)   │   GET /stream(全局唯一,被占返回 409)
      ┌───────────────┬──────┴────────┬───────────────┐
      │  客户端 Mac A  │  客户端 Mac B  │  客户端 Mac C   │
      │  BlackHole    │  BlackHole    │  BlackHole    │
      │  → Zoom       │  → 微信        │  → Teams      │
      └───────────────┴───────────────┴───────────────┘
          同一时间只有一台客户端持有串流(先到先得)
```

服务端只做监听,客户端事件驱动:平时没有任何音频流,**说话触发语音激活(VAD)后,客户端才通过 API 自动拉起串流**;静音 1.5 秒自动收流、回到待机。数据流细节:
